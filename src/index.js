...
const { beforeRunHook, emitHook, getCompilerHooks, normalModuleLoaderHook, additionalAssetsHook } = require('./hooks');
...
class ImportMapPlugin {
    ...
    apply (compiler) {
        ...
        const moduleAssets = {};
        const importMapFileName = resolve(compiler.options.output.path, this.options.fileName);
        const importMapAssetId = relative(compiler.options.output.path, importMapFileName);
        const baseImportMapAssetId = 'import-map-plugin/base-import-map.json';
        const beforeRun = beforeRunHook.bind(this, { emitCountMap, importMapFileName });
        const additionalAssets = additionalAssetsHook.bind(this, {
            baseImportMap: this.options.baseImportMap,
        ...
        if (webpack.version.startsWith('4')) {
            compiler.hooks.emit.tap(hookOptions, compilation => {
                compilation.hooks.additionalAssets.tapAsync(pluginName, additionalAssets);
                emit(compilation);
            });
        } else {
            compiler.hooks.thisCompilation.tap(hookOptions, (compilation) => {
                compilation.hooks.additionalAssets.tapAsync(pluginName, (callback) => additionalAssets(compilation, callback));
